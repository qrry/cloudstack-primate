...
export default {
  ...
  methods: {
    ...
    updateRecord (record) {
      const updateRecodeAction = this.actions.find(action => action.key === 'update')
      if (!updateRecodeAction || !updateRecodeAction.component) {
        return
      }
      this.currentAction = updateRecodeAction
